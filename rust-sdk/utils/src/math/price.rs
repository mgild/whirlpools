...
pub fn price_to_sqrt_price(price: f64, decimals_a: u8, decimals_b: u8) -> U128 {
    ...
    let sqrt_price = floor(sqrt(price * power) * Q64_RESOLUTION) as u128;
    sqrt_price.into()
}
...
pub fn sqrt_price_to_price(sqrt_price: U128, decimals_a: u8, decimals_b: u8) -> f64 {
    let power = pow(10f64, decimals_a as f64 - decimals_b as f64);
    let sqrt_price_u128 = <u128>::from(sqrt_price) as f64;
...
mod tests {
    ...
    fn test_sqrt_price_to_price() {
        assert_eq!(
            sqrt_price_to_price(1844674407370955161600u128.into(), 8, 6),
            100.0
        );
        assert_eq!(
            sqrt_price_to_price(184467440737095516160u128.into(), 6, 6),
            100.0
        );
        assert_eq!(
            sqrt_price_to_price(18446744073709551616u128.into(), 6, 8),
            100.0
        );
    }
